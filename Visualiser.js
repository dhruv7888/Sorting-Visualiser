...
async function BubbleSort()
{
    for(let step=0 ; step < n-1 ; step++)
    {
        ...
        {
            await assign(j,heights[j],"red");
            await assign(j+1,heights[j+1],"red");
            ...
            await assign(j,heights[j],"aqua");
            await assign(j+1,heights[j+1],"aqua");
        }
        await assign(n-step-1,heights[n-step-1],"violet");
    }
    await assign(0,heights[0],"violet");
}
...
async function InsertionSort()
{
    ...
    for (i = 1; i < n; i++)
    {
        ...
        while (j >= 0 && heights[j] > key)
        {
            ...
            await assign(j,heights[j],"red");
            await assign(j+1,heights[j+1],"red");
            await assign(j,heights[j],"violet");
            await assign(j+1,heights[j+1],"violet");
            j--;        
        }
        heights[j + 1] = key;
        await assign(j+1,heights[j+1],"violet");
        while((j>=0)&&(bars[j].style.color!=="violet"))
        {
            await assign(j,heights[j],"violet"); 
            j--;   
        }
...
async function Merge(start,mid,end)
{
    ...
    {
        await assign(start+i,heights[start+i],"red");
        arrayl.push(heights[start+i]);
    }
    for(let i=0;i<sizer;i++)
    {
        await assign(mid+1+i,heights[mid+1+i],"red");
    ...
    while(indexl<sizel && indexr<sizer)
    {
        if(arrayl[indexl]<=arrayr[indexr])
        {
            ...
            await assign(indexmrg,heights[indexmrg],"red");
            indexl++;    
        }
        else
        {
        ...
        await assign(indexmrg,heights[indexmrg],"violet");
        indexmrg++;
    }
    while(indexl<sizel)
    {
...
async function SelectionSort()
{
    ...
    {
        let mini_ind=i;
        await assign(i,heights[i],"red");
        for(let j=i+1;j<n;j++)
        {
            await assign(j,heights[j],"red");
            if(heights[j]<heights[mini_ind])
            {
                await assign(mini_ind,heights[mini_ind],"violet");    
                mini_ind=j;
            }        
            else
            {
        ...
        await assign(mini_ind,heights[mini_ind],"violet");    
        await assign(i,heights[i],"violet");
    }
}
